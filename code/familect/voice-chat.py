...
SPK_DEVICE  = "spk_sv"     # ALSA playback device
...
class FarewellComplete(Exception):
    """Raised when the AI has said goodbye and the session should end cleanly."""
...
class VoiceClient:
    ...
    def _mic_cb(self, indata, frames, _time, _status):
        ...
        # Only count as user speech when the AI is not speaking, to prevent
        # speaker bleed from constantly resetting the silence timer.
        if energy > MIC_THRESHOLD and not self._ai_speaking:
    ...
    async def _farewell_monitor(self):
        """On SIGUSR1: ask the AI to say goodbye, wait for it to finish, then exit."""
        await self._setup_done.wait()
        ...
        await self._ws.send(json.dumps({
            "clientContent": {
                "turns": [{"role": "user", "parts": [{"text": "[the user has removed their card — warmly say goodbye and end the conversation]"}]}],
                "turnComplete": True,
            }
        }))
        # Wait for the AI to start speaking, then for it to finish (max 12 s)
        await asyncio.sleep(0.8)
        deadline = asyncio.get_event_loop().time() + 12
        while self._ai_speaking and asyncio.get_event_loop().time() < deadline:
            await asyncio.sleep(0.2)
        # Small tail so the last audio chunk drains from the play buffer
    ...
    async def run(self):
        ...
        self._running = True
        # SIGUSR1 → begin farewell sequence
        ...
        try:
            async with websockets.connect(f"{WS_URL}?key={self.api_key}", max_size=8 << 20) as ws:
                ...
                farewell_t = asyncio.create_task(self._farewell_monitor())
                done, pending = await asyncio.wait(
                    [recv_t, send_t, monitor_t, farewell_t],
                ...
                for t in done:
                    exc = t.exception()
                    if exc and not isinstance(exc, FarewellComplete):
                        raise exc
        finally:
            self._running = False
